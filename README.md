...
First we need to define the functions we want to call between the enclave and host. To do this we create a `dcr_proxy.edl` file:

```edl
enclave {
    trusted {
        public void enclave_dcr_proxy();
...
In this `dcr_proxy.edl` file we define two different functions.
...
```bash
oeedger8r --search-path /opt/openenclave/include --search-path /opt/openenclave/include/openenclave/edl/sgx ../dcr_proxy.edl --untrusted
```
...
```cmd
oeedger8r --search-path c:\openenclave\include --search-path c:\openenclave\include\openenclave\edl\sgx ..\dcr_proxy.edl --untrusted
```

This command compiles the `dcr_proxy.edl` file and generates the following files within the host directory:

| file | description |
|---|---|
| host/dcr_proxy_args.h | Defines the parameters that are passed to all functions defined in the edl file |
| host/dcr_proxy_u.c | Contains the `enclave_dcr_proxy()` function with the marshaling code to call into the enclave version of the `enclave_dcr_proxy()` function |
| host/dcr_proxy_u.h | Function prototype for `enclave_dcr_proxy()` function |
...
```bash
oeedger8r --search-path /opt/openenclave/include --search-path /opt/openenclave/include/openenclave/edl/sgx ../dcr_proxy.edl --trusted
```

On Windows, if the openenclave package is installed at `c:\openenclave` and the developer is looking to build an enclave application targeting SGX, the developer would need to run the following command:

```cmd
oeedger8r --search-path c:\openenclave\include --search-path c:\openenclave\include\openenclave\edl\sgx ..\dcr_proxy.edl --trusted
```

This command compiles the `dcr_proxy.edl` file and generates the following files within the enclave directory:
| file | description |
|---|---|
| enclave/dcr_proxy_args.h | Defines the parameters that are passed to all functions defined in the edl file |
| enclave/dcr_proxy_t.c | Contains the `host_dcr_proxy()` function with the marshaling code to call into the host version of the `host_dcr_proxy()` function |
| enclave/dcr_proxy_t.h | function prototype for `host_dcr_proxy()` function |

## Enclave component

This section shows how to develop and build a simple enclave called dcr_proxy.

### Develop an enclave

An enclave exposes its functionality to the host application in the form of a set of trusted methods that are defined in the `dcr_proxy.edl` file and implemented within the enclave project.

The dcr_proxy sample implements a single function named `enclave_dcr_proxy` which is called by the host. All it does is print out a message and then call back to the host. No parameters are passed in this sample for simplicity.

The full source for the enclave implementation is here: [dcr_proxy/enclave/enc.c](enclave/enc.c)
...
void enclave_dcr_proxy()
{
    ...
    fprintf(stdout, "Running enclave code from the enclave\n");

    // Call back into the host
    oe_result_t result = host_dcr_proxy();
    if (result != OE_OK)
    {
        fprintf(stderr, "Call to host_dcr_proxy failed: result=%u (%s)\n", result,
...
An enclave exposes its functionality via a set of methods defined in the `dcr_proxy.edl` file and implemented here. The only implemented function in the enclave in this sample is `enclave_dcr_proxy`.

```c
fprintf(stdout, "Running host code from the enclave\n");
```
...
This calls the marshaling function that is generated from the `dcr_proxy.edl` file which in turn calls into the function within the host. Even though the `host_dcr_proxy()` function is a `void` this call can still fail within the marshaling code itself and so we should always validate it. If `host_dcr_proxy()` were to return a value, it would actually be passed back as an out parameter of the function.
...
| File | Description |
| --- | --- |
| enc.c | Source code for the enclave `enclave_dcr_proxy` function |
| CMakeLists.txt | CMake file to build the enclave |
| Makefile | Makefile used to build the enclave |
| dcr_proxy.conf | Configuration parameters for the enclave |
...
| dcr_proxyenc | built and linked enclave executable |
| dcr_proxyenc.signed | signed version of the enclave executable |
| dcr_proxy_args.h | Defines the parameters that are passed to all functions defined in the edl file |
| dcr_proxy_t.c | Contains the `host_dcr_proxy()` function with the marshaling code to call into the host version of the `host_dcr_proxy()` function |
| dcr_proxy_t.h | function prototype for `host_dcr_proxy()` function |
| dcr_proxy_t.o | compiled marshaling code for dcr_proxy_t.c |
...
Only the signed version of the enclave `dcr_proxyenc.signed` is loadable on Linux as enclaves are required to be digitally signed.
...
`dcr_proxyenc` is the resulting enclave executable (unsigned).
...
The signing process also reads the `dcr_proxy.conf` file which describes important parameters associated with with enclave.
...
In this section we will cover how to develop a host to load and run the dcr_proxy enclave we built above.
...
The full source for the host implementation is here: [dcr_proxy/host/host.c](host/host.c)
...
int main(int argc, const char* argv[])
{
    ...
    if (result != OE_OK)
    {
        fprintf(stderr, "oe_create_dcr_proxy_enclave(): result=%u (%s)\n", result, oe_result_str(result));
        goto exit;
    }
    ...
    if (result != OE_OK)
    {
        fprintf(stderr, "calling into enclave_dcr_proxy failed: result=%u (%s)\n", result, oe_result_str(result));
        goto exit;
    }
...
This is the actual host function that the enclave calls into. The function is defined in the `dcr_proxy.edl` file and implemented here.
...
This `oe_create_dcr_proxy_enclave` function is generated by oeedger8r.
...
The dcr_proxy sample creates an enclave by calling `oe_create_dcr_proxy_enclave` with the path to the signed enclave library file, which happens to be passed as the first parameter to the launching application.
...
This function calls into the host marshaling function that is generated from the `dcr_proxy.edl` file. It handles the code that marshals any parameters, and calls the function within the enclave itself. In this sample, we do not have any actual function parameters. Even though the function `enclave_dcr_proxy()` is a `void` return type, the marshaling code itself can fail, so we need to validate the return code associated with it. If `enclave_dcr_proxy()` were to return a value, this would be passed back as an out parameter.
...
| File | Description |
| --- | --- |
| host.c | Source code for the host `host_dcr_proxy` function, as well as the executable `main` function. |
...
| dcr_proxyhost | built and linked host executable |
| dcr_proxy_args.h | Defines the parameters that are passed to all functions defined in the edl file |
| dcr_proxy_u.c | Contains the `enclave_dcr_proxy()` function with the marshaling code to call into the enclave version of the `enclave_dcr_proxy()` function |
| dcr_proxy_u.h | Function prototype for `enclave_dcr_proxy()` function |
| dcr_proxy_u.o | compiled dcr_proxy_u.c source file |
...
dcr_proxy sample can run under OE simulation mode.

On Linux, to run the dcr_proxy sample in simulation mode from the command like, use the following:

```bash
./host/dcr_proxyhost ./enclave/dcr_proxyenc.signed --simulate
```

On Windows, to run the dcr_proxy sample in simulation mode from the command like, use the following:
...
In this tutorial, you built and ran the dcr_proxy sample. Next, try out more OE SDK samples on [Linux](../README.md#samples).
